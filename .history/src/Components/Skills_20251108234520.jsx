...
function SkillIcon({ skill, index, isVisible }) {
  return (
    <div
      className={`group relative flex flex-col items-center justify-center p-6 rounded-3xl transition-all duration-700 cursor-pointer
      bg-gradient-to-br from-white/10 to-gray-800/30 dark:from-gray-800 dark:to-gray-900 shadow-[0_4px_20px_rgba(0,0,0,0.2)]
      hover:shadow-[0_0_25px_rgba(255,255,255,0.2)] hover:-translate-y-3
      ${isVisible ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
      style={{ animationDelay: `${index * 0.05}s` }}
    >
      {/* Glow Background */}
      <div
        className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 blur-3xl transition-all duration-700"
      ...
      <img
        src={skill.icon}
        alt={skill.name}
        className="relative w-14 h-14 z-10 transition-all duration-500 group-hover:scale-125 group-hover:rotate-6"
      />
...
export default function Skills() {
  ...
  return (
    <section
      id="skills"
      className="relative py-24 px-6 overflow-hidden bg-gradient-to-br from-gray-100 via-gray-200 to-gray-100 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950"
    >
      {/* Decorative Glow */}
      <div className="absolute -top-20 -left-20 w-72 h-72 bg-blue-600/20 blur-[120px] rounded-full"></div>
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-purple-600/10 blur-[140px] rounded-full"></div>
      ...
      <div className="max-w-7xl mx-auto text-center relative z-10">
        ...
        <div className="space-y-16">
          {Object.entries(skillsData).map(([category, skills], i) => (
            <div key={category}>
              <h3 className="text-2xl font-semibold mb-8 text-gray-900 dark:text-white capitalize">
                {category}
              </h3>
