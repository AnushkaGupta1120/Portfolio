...
function SkillIcon({ skill, index, isVisible }) {
  return (
    <div
      ...
      bg-white dark:bg-indigoCard shadow-md hover:shadow-glow hover:-translate-y-3
    ...
    >
      ...
      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 translate-y-full opacity-0 group-hover:translate-y-2 group-hover:opacity-100 transition-all duration-300 z-20">
        <div className="bg-gradient-to-r from-accent to-mutedIndigo text-white text-xs font-semibold py-1 px-3 rounded-lg shadow-md whitespace-nowrap">
          {skill.name}
        </div>
      </div>
    </div>
...
export default function Skills() {
  ...
  return (
    <section
      id="skills"
      className="relative py-24 px-6 overflow-hidden bg-gray-50 dark:bg-navy transition-colors duration-500"
    >
      <div className="max-w-7xl mx-auto text-center relative z-10">
        <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-accent to-mutedIndigo bg-clip-text text-transparent">
          Technical Skills
        </h2>
        ...
        <div className="space-y-16">
          {Object.entries(skillsData).map(([category, skills], i) => (
            <div key={category}>
              <h3 className="text-2xl font-semibold mb-8 text-gray-900 dark:text-light capitalize">
                {category}
              </h3>
