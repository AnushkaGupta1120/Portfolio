...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.message) {
      return triggerPopup("error", "Please fill out all fields.");
    }
    ...
    try {
      ...
      if (data.success) {
        triggerPopup("success", "Message sent successfully!");
        setFormData({ name: "", email: "", message: "" });
      } else {
        triggerPopup("error", "Failed to send message.");
      }
    } catch {
      triggerPopup("error", "Something went wrong.");
    }
  ...
  const triggerPopup = (type, message) => {
    setShowPopup({ show: true, type, message });
    setTimeout(() => setShowPopup({ show: false, type: "", message: "" }), 3000);
  };
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            Whether you have an opportunity, collaboration idea, or just want to say hi — my inbox is always open.
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          >
            ...
            <input type="hidden" name="_to" value="anushkaguptawork01@gmail.com" />
            ...
            <motion.button
              type="submit"
              disabled={loading}
              whileHover={!loading && { scale: 1.05, boxShadow: "0 0 30px var(--teal-1)" }}
  ...
  return (
    <>
      <motion.div
        ...
        style={{ background: "radial-gradient(circle, rgba(0,234,255,0.12), rgba(0,234,255,0.03), transparent)" }}
        animate={{ y: [0, 25, 0], opacity: [0.3, 1, 0.3] }}
        transition={{ duration: 8, repeat: Infinity }}
      />
      <motion.div
        className="absolute bottom-10 right-10 w-[340px] h-[340px] rounded-full"
        style={{ background: "radial-gradient(circle, rgba(0,255,198,0.1), rgba(0,255,198,0.03), transparent)" }}
        animate={{ y: [0, -25, 0], opacity: [0.3, 1, 0.3] }}
...
function ContactInfoList({ contactInfo }) {
  return (
      ...
      {contactInfo.map((item, i) => {
        ...
        return (
          <motion.a
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            whileHover={{ scale: 1.03, boxShadow: "0 0 20px rgba(0,234,255,0.25)" }}
...
function Popup({ type, message }) {
  return (
    <motion.div
      ...
      className={`fixed bottom-10 right-10 px-6 py-3 rounded-xl shadow-lg backdrop-blur-md z-50 
      ${type === "success" ? "bg-green-400 text-black" : "bg-red-500 text-white"}`}
    >
      {message}
    </motion.div>
  );
